...
public class UsuarioController {
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'CLIENTE')")
    public ResponseEntity<UsuarioResponseDTO> getMe(@AuthenticationPrincipal JwtUserDetails userDetails) {
        UsuarioResponseDTO dto = UsuarioMapper.toDto(usuarioService.buscarPorId(userDetails.getId()));
    ...
    @Operation(
            summary = "Atualizar email e/ou telefone do usuário logado.",
            description = "Permite que o usuário logado altere seu email e/ou telefone. "
                    + "Não é obrigatório alterar ambos. Os campos são validados para unicidade e não podem ser iguais aos atuais.",
            tags = {"Usuarios"},
            security = @SecurityRequirement(name = "security"),
            responses = {
                    @ApiResponse(
    ...
    @PatchMapping(value = "/me/contact", produces = {MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE},
            consumes = {MediaType.APPLICATION_JSON_VALUE, MediaType.APPLICATION_XML_VALUE})
    @PreAuthorize("hasAnyRole('ADMIN', 'CLIENTE')")
    public ResponseEntity<UsuarioResponseDTO> updateMe(@AuthenticationPrincipal JwtUserDetails userDetails,
                                                       @Valid @RequestBody AtualizarContatoDTO dto) {
        UsuarioResponseDTO response = UsuarioMapper.toDto(usuarioService.updateEmailAndOrTelefone(userDetails.getId(), dto));
